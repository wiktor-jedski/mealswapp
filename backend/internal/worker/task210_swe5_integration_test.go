...
// IT-ARCH-004-002 and IT-ARCH-004-005, ARCH-004, DESIGN-004, and SW-REQ-021/SW-REQ-022/SW-REQ-030.
func TestTask210WorkerPublishesPartialAlternativesOnLaterSolverFailure(t *testing.T) {
	...
	manager := queue.NewJobQueueManager(client, queue.Config{
		...
		VisibilityTimeout: queue.DefaultVisibilityTimeout, ReadBlock: 10 * time.Millisecond,
	...
	if solver.calls != 3 {
		t.Fatalf("solver calls = %d, want two lexicographic passes plus later failure", solver.calls)
	}
...
// Solve returns both lexicographic passes for one valid alternative, then
// simulates the bounded timeout that must preserve the validated partial result.
