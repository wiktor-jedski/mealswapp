...
- `MealEntity`: owns single-meal and recipe-meal references plus meal-level physical and preparation metadata.
...
- `interface FoodItemEntity { id: UUID; name: string; physicalState: PhysicalState; prepTimeMinutes: number; averageUnitWeightGrams: number; macrosPer100: MacroValues; micros: MicroValues; categoryTags: TagEntity[]; functionalityTags: TagEntity[]; imageUrl?: string }`
- `interface MealEntity { id: UUID; type: "single" | "recipe"; itemId?: UUID; recipeItems?: RecipeIngredientEntity[]; physicalState: PhysicalState; prepTimeMinutes: number; averageUnitWeightGrams: number }`
...
3. Validate every micronutrient key against active `MicronutrientVocabularyEntry` records before insert or update; reject aliases such as `Na` when the canonical key is `Sodium`.
4. Store micronutrients as supplemental display/export data only; repository methods that build similarity inputs must return only protein, carbohydrates, and fat.
5. Use repository methods as the only data access path for domain services.
6. Apply user scoping in repository queries whenever custom items, saved meals, profile data, or history are requested.
7. For recipe meals, load ingredients and sum each ingredient macro after scaling by ingredient quantity.
8. Convert metric values to imperial only at the repository boundary when `RepositoryContext.unitSystem = "imperial"`.
9. Use raw SQL with parameter binding through `pgx` or `lib/pq`; never concatenate user input into SQL.
10. Maintain indexes for item name, category tags, functionality tags, micronutrient vocabulary keys, and common filter columns.
11. Return domain entities with normalized macros, validated micronutrients, and hydrated tag lists for callers.
...
- `recipe_cycle_error`: prevent recipes from directly or indirectly containing themselves.
...
- `type MealRepository interface { GetByID(ctx context.Context, id UUID, rc RepositoryContext) (MealEntity, error); CalculateMacros(ctx context.Context, mealID UUID) (MacroValues, error) }`
- `type TagRepository interface { List(ctx context.Context, kind string) ([]TagEntity, error); Upsert(ctx context.Context, tag TagEntity) (UUID, error) }`
...
- `func BuildSimilarityMacroVector(item FoodItemEntity) MacroValues`
