...
**Static aspects covered:** AuthView, RegisterView, LoginView, AuthSessionStore, AuthApiClient, ConsentGate, OAuthEntryPoint, AuthenticatedActionGuard.
...
- `AuthApiClient`: owns generated-contract calls for CSRF retrieval, registration, login, logout, refresh/session recovery, profile/session probing, and OAuth start URLs.
...
3. When `AuthView` opens, choose `login` mode unless the invoking action explicitly requests registration. Keep the current search state intact while the auth surface is visible.
4. On registration submission, validate local form completeness, password confirmation, current consent checkbox acceptance, and consent version presence before requesting CSRF.
5. For registration, fetch a fresh CSRF token, submit generated registration payload with consent versions and `credentials: "include"`, map duplicate-email, stale-consent, validation, lockout, and network failures through ARCH-017, then refresh session/profile and entitlement state on success.
6. On login submission, validate local form completeness before requesting CSRF.
7. For login, fetch a fresh CSRF token, submit generated login payload with `credentials: "include"`, map invalid credentials to a generic user-facing message, surface lockout/rate-limit retry timing when the server provides safe metadata, then refresh session/profile and entitlement state on success.
8. For OAuth login, navigate to the generated provider start URL. On return to the SPA, run session/profile and entitlement refresh; do not infer success from URL parameters alone.
9. For logout, fetch a fresh CSRF token, submit logout with `credentials: "include"`, clear frontend-safe session state to `anonymous`, and preserve anonymous Catalog Search state.
10. `AuthenticatedActionGuard` checks `AuthSessionProjection` before protected actions. Anonymous or expired users see sign-in/register guidance and the pending action is retained as a retry callback.
11. After successful authentication, run the retained protected-action callback exactly once and clear it, so checkout can be retried from a real HttpOnly-cookie browser session.
12. Registration through email/password may create an unverified login method. The UI must display verified-login restriction feedback from server state and must not locally override paid-feature eligibility.
13. Never store or expose access tokens, refresh tokens, raw password values after submission, provider secrets, Stripe customer IDs, or payment-card fields in frontend state.
