...
- Add generated-contract frontend clients for CSRF token retrieval, email/password registration with consent, login, logout, refresh/session recovery, and OAuth start/callback entry points where the backend contract already exists.
- Add user-facing Svelte auth state and UI: login/register mode switching, consent checkboxes, generic invalid-credential errors, duplicate-email feedback, locked/rate-limited feedback, authenticated-session display, logout, and anonymous fallback for Catalog Search. Keep medical-disclaimer information in the Terms of Service and future About section rather than the auth surface.
...
- Lint auth/profile/billing OpenAPI contract usage, regenerate frontend types if contract drift is found, and typecheck generated DTO/request helpers against their production `fetch` consumers.
...
- Reshape `config.OAuthConfig` into provider-specific nested configuration, such as `OAuthConfig.Google GoogleOAuthConfig` and `OAuthConfig.Apple AppleOAuthConfig`, so each provider's credentials, callback URL, scopes, and validation rules are explicit rather than stored in Google-oriented or falsely generic fields.
...
### Phase 10: Mixed-Integer Daily Diet Alternatives

- Replace the continuous-only CLP optimization boundary with a pinned mixed-integer solver boundary, using COIN-OR CBC unless implementation validation identifies a documented blocker. Preserve the existing external child-process isolation, bounded diagnostics, private temporary files, cancellation, whole-job deadline, and worker-only execution model.
- Extend the optimization model with binary selection variables linked to continuous `g`/`ml` quantity variables. Enforce product-approved minimum and maximum selected-item counts plus meaningful minimum portions so cardinality cannot be satisfied with negligible quantities.
- Preserve server-derived Protein, Carbohydrate, and Fat targets, user-selected tolerance, explicit exclusions, the maximum of three alternative combinations, repository-owned result names, authoritative macro/calorie recalculation, and user-scoped Redis job polling.
- Replace the calorie-first objective with a lexicographic target-closeness policy. First minimize normalized absolute deviation from the server-derived Protein, Carbohydrate, and Fat targets inside the user-selected tolerance band; tolerance is a feasibility boundary, not a destination, so a closer feasible solution must beat one on the lower tolerance boundary. Then minimize calories among equally close solutions, and only then apply diversity and prior-alternative exclusion without allowing either preference to worsen target closeness. Every handoff between CBC objective stages must preserve the prior optimum with a documented tolerance-aware bound derived from solver and publication precision; never copy a parsed floating-point optimum into an exact equality constraint. Validate objective values, selection variables, and published quantities at the authoritative solution boundary, and align SW-REQ-022 plus DESIGN-004 with this objective order before implementation.
- Package and checksum-pin the supported CBC executable for local development and the optimizer image; add startup version/readiness validation and update solver configuration, verification scripts, deployment documentation, and safe error classification without exposing solver diagnostics to users.
- Add model, serialization, parser, timeout, infeasible, target-closeness, cardinality, minimum-portion, diversity, worker integration, performance, browser, and UAT coverage. Include a regression fixture where a wide tolerance admits both a proportional lower-bound solution and a solution closer to the targets; the closer solution must win even when the lower-bound solution has fewer calories. Add a CBC regression in which the first objective returns rounded quantities and a small residual: later objective stages must remain feasible, preserve the earlier optimum within the approved bound, and never publish a false `solver_infeasible`. Verify the configured candidate-set ceiling and repeated alternative solves remain within the whole-job deadline.
- Exit criteria: generated alternatives minimize normalized macro-target deviation before calories and diversity, tolerance-aware objective locks prevent rounded CBC output from creating false infeasibility, user tolerance no longer drives feasible results deliberately to its lower boundary, alternatives obey the accepted item-count and minimum-portion policy, no negligible filler item can satisfy cardinality, the worker publishes up to three validated human-readable alternatives through the existing API lifecycle, and CBC packaging/readiness plus integration/browser/UAT evidence pass.

### Phase 11: Macro-Target Diet Generation
...
- Reuse the Phase 07 Redis queue and worker lifecycle plus the Phase 10 mixed-integer solver, whole-job deadline, admission limits, polling, solution validation, and result rendering rather than introducing another optimization engine.
