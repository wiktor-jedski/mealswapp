...
- Open review finding on 2026-07-03: checkout creation persists application-level `Idempotency-Key` records after a successful Stripe Checkout Session response, but `StripeCheckoutGateway.CreateCheckoutSession` does not pass an idempotency key to Stripe's `/v1/checkout/sessions` request. A timeout or connection failure after Stripe creates the session but before the application stores the local idempotency response can lead to a retry creating another Stripe Checkout Session. Add provider-side idempotency by deriving a Stripe idempotency key from the authenticated user scope, checkout route, and client `Idempotency-Key`, pass it to the Stripe API call, and cover the ambiguous-response retry case with a focused checkout test.
- Open review finding on 2026-07-03: Stripe reconciliation maps `active`/`trialing`, `past_due`/`unpaid`/`incomplete_expired`, and `canceled`/`cancelled`, but skips other Stripe subscription statuses such as `incomplete` or `paused` without changing local entitlement state or emitting an operator-visible warning. If local state is stale `paid:active`, reconciliation will not repair it for those statuses. Decide the local entitlement mapping for each supported Stripe status and log skipped unknown statuses with sanitized metadata so billing drift is observable.
- Open review finding on 2026-07-03: Stripe webhook dead-letter persistence calls `sanitizedErrorMessage`, but the function reduces every non-nil cause to the generic string `stripe webhook processing failed`. This avoids leaking raw provider payloads or secrets, but makes the `error_message` column redundant with `failure_category` and weakens operator triage. Replace it with a bounded safe failure code/category, such as repository error kind or processing stage, or remove the column if dynamic diagnostic text is intentionally forbidden. Keep raw Stripe payloads, emails, card data, SQL details, and secrets out of dead-letter rows.
- Open review finding on 2026-07-03: `entitlementStatusForStripeEvent` handles the main checkout, invoice success/failure, and subscription created/updated/deleted events, but does not explicitly handle all entitlement-relevant Stripe subscription events and statuses. Stripe statuses such as `incomplete` and `paused`, and events such as `customer.subscription.paused`, `customer.subscription.resumed`, and `invoice.payment_action_required`, currently fall through as no-ops. Decide whether each should collapse into the existing local entitlement states (`active`, `past_due`, `cancelled`, `expired`) or require new local/API states such as `paused`, `incomplete`, or `payment_action_required`. If the model expands, update `billingRecoveryState`, OpenAPI `EntitlementState`/`BillingRecoveryState`, generated frontend types, and subscription recovery messaging; if the model stays small, deliberately map every blocked recoverable Stripe state to an existing recovery state. Add focused webhook/status tests and log sanitized no-op handling for ignored but recognized billing events so stale `paid:active` access is not preserved silently.
- Open review finding on 2026-07-03: remove the package-level `max` helper from `backend/internal/entitlement/usage_limiter.go`. The backend module targets Go 1.25, so remaining-count calculations in `usage_limiter.go` and `status.go` can use the built-in `max` directly instead of a local helper with the same name. Keep the existing clamping behavior for negative remaining usage.
- Open review finding on 2026-07-03: fix `UsageLimiter.CheckSearchAllowed` so inactive paid entitlements do not bypass the free-tier usage cap for free-scope searches. `EntitlementManager` allows free features before checking active paid status, then `UsageLimiter` skips counting whenever `Tier != "free"` or `Status != "active"`. As a result, `paid:past_due` or `paid:cancelled` users can run Catalog or single-input Substitution without the 3-per-24h free limit. Treat inactive paid/cancelled/expired users as effective free users for free-scope usage accounting, or make the policy explicit if they should receive uncapped free access. Add regression tests for `paid:past_due` and `paid:cancelled` free-scope searches.
