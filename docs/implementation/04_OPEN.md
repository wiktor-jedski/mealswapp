...
- Deferred to Phase 07: `frontend/src/lib/stores/search.ts` currently uses one broad `SearchState` interface with mode-specific fields (`substitutionInputs`, `dailyDietId`) guarded by `setMode()` and `buildSearchRequest()`. This is acceptable for the Phase 05 Daily Diet scaffold because transition helpers and request-building tests prevent invalid API payloads. Phase 07 must refactor frontend search state into a discriminated union or nested per-mode state before implementing full Daily Diet behavior, so impossible Catalog/Substitution/Daily Diet combinations are prevented by types instead of only by transition helpers and tests.
