...
- Planned as Phase 06.01 tasks 177-188: manual webapp acceptance of Phase 06 subscription checkout is blocked because the backend authentication API exists, but the frontend does not provide a sign-in/register surface that creates an authenticated browser session. Phase 06.01 adds ARCH-018/DESIGN-018 frontend auth session architecture, generated auth client, login/register/consent/disclaimer UI, authenticated checkout guard, authenticated Search/Subscription sidebar navigation, browser workflow coverage, SWE.5 obligations, and UAT evidence. A local development auth shortcut is explicitly not accepted as project-owner checkout UAT evidence.
