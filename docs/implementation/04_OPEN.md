...
- Open review finding on 2026-07-03: manual webapp acceptance of Phase 06 subscription checkout is blocked because the backend authentication API exists, but the frontend does not provide a sign-in/register surface that creates an authenticated browser session. Current frontend components only probe authenticated state and fall back to anonymous behavior. Add a frontend login/register flow, or document and implement a local development auth shortcut, before relying on project-owner UAT steps that start checkout from the webapp.
