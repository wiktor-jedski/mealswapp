...
- Backend `internal/userdata` coverage: `98.1%`; `export.go` is `104/104` statements (`100.0%`) in the focused package profile. The phase-wide profile measures `130/134` export statements because four typed-projection/bytes-buffer defensive CSV branches are not inducible; the dated owned deviation is recorded in `docs/implementation/04_OPEN.md`.
- Frontend full coverage: `537` tests, `2,825` expectations, `All files` at `95.19%` functions and `96.12%` lines; `account-data-client.ts` is `100.00%` functions and `100.00%` lines. The decoder regression covers empty, ASCII, and emoji micronutrient keys against OpenAPI `propertyNames` bounds using Unicode code-point counting. The exact contract in `docs/implementation/04_OPEN.md` matches this output.
