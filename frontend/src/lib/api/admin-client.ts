...
async function decodeItem(responsePromise: Promise<Response> | Response, expectedStatus: number, mutationResponse = false): Promise<AdminItem> {
	...
	try {
		...
		if (!optionalBoundedString(value.densitySourceProvider, 64) || !optionalPositive(value.averageUnitWeightGrams) || !optionalPositive(value.averageServingVolumeMilliliters) || !optionalPositive(value.densityGramsPerMilliliter) || !optionalBoundedString(value.densitySourceFoodId, 200) || (value.densitySourceKind !== undefined && !["imported", "manual", "estimated"].includes(String(value.densitySourceKind))) || !optionalBoundedString(value.imageUrl, 2048) || (value.imageUrl !== undefined && !safeUriReference(value.imageUrl))) throw malformed(response.status);
		...
		if (value.densitySourceKind === "imported" && (!value.densitySourceProvider || !value.densitySourceFoodId)) throw malformed(response.status);
		if ((value.densitySourceKind === "manual" || value.densitySourceKind === "estimated") && (value.densitySourceProvider !== undefined || value.densitySourceFoodId !== undefined)) throw malformed(response.status);
