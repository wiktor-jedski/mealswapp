<script lang="ts">
  ...
  // Implements DESIGN-018 OAuthEntryPoint configured provider entry and callback-return refresh UI.
  ...
  let providers = $state(configuredOAuthProviders());
      ...
      <div class="grid gap-2 sm:grid-cols-2">
        {#each providers as provider}
          <button
            ...
            aria-busy={busyProvider === provider ? "true" : "false"}
          ...
          >
            ...
            <span class="whitespace-nowrap">{providerLabels[provider]}</span>
          </button>
        {/each}
      </div>
    {/if}
