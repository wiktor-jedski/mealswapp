...
class Phase08BackendCoverageContractTests(unittest.TestCase):
	...
	def test_task277_backend_metrics_are_current(self) -> None:
		...
		self.assertEqual(rows["internal/externaldata/openfoodfacts.go"][0], check.GoCoverage(184, 192, "206.62-208.12,228.10-230.4,248.14-250.12,252.26-254.12,260.2-260.17"))
		self.assertEqual(rows["internal/externaldata/rate_limit.go"][0], check.GoCoverage(186, 188, "357.14-359.3,407.46-409.3"))
...
class FrontendCoverageContractTests(unittest.TestCase):
	...
	def test_task277_frontend_metrics_are_current(self) -> None:
		...
		self.assertEqual(rows["src/lib/admin-workflows.ts"][1], check.FrontendCoverage("94.44", "98.94", "-"))
